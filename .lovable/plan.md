...
2. Evento activo — lo establece únicamente el sistema, de forma automática, al llegar la fecha y hora exactas y superar la evaluación final: se cobra, se confirma a quien pagó, se abre el acceso y la sala se muestra solo a participantes confirmados. Desde aquí los datos de sala quedan bloqueados y se usa la última versión guardada. El administrador no puede activarlo a mano ni adelantar la hora.
...
Cambios manuales permitidos: evento activo → evento iniciado, evento iniciado → evento finalizado, y cancelar desde inscripciones activas o evento activo. La activación nunca es manual. El servidor rechaza cualquier intento de poner un evento en activo a mano, de finalizar sin haber iniciado, de iniciar sin haber estado activo y de editar la sala una vez bloqueada. Toda acción manual queda auditada con administrador, fecha/hora, estado anterior y nuevo.
